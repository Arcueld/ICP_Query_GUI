...
class BatchQueryWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            summary_result = self._generate_summary_result(
    ...
    def _generate_summary_result(self, successful_queries, failed_queries):
        ...
        for query_result in self.batch_results:
            if query_result["success"] and query_result["result"].get("params", {}).get(
                "list"
            ):
...
class ConfigDialog(QDialog):
    ...
    def init_ui(self):
        ...
        query_layout.addWidget(QLabel("启用风控延时:"), 1, 0)
        self.risk_delay_enable = QCheckBox()
        query_layout.addWidget(self.risk_delay_enable, 1, 1)

        query_layout.addWidget(QLabel("延时秒数:"), 2, 0)
        self.risk_delay_seconds = QSpinBox()
        self.risk_delay_seconds.setRange(1, 600)
        self.risk_delay_seconds.setValue(1)
        query_layout.addWidget(self.risk_delay_seconds, 2, 1)
        query_layout.addWidget(QLabel("(每次查询前 sleep，单位秒)"), 2, 2)
    ...
    def load_config(self):
        try:
            ...
            self.risk_delay_enable.setChecked(
                getattr(query, "risk_delay_enable", False)
            )
            self.risk_delay_seconds.setValue(
                max(1, int(getattr(query, "risk_delay_seconds", 1)))
            )
    ...
    def save_config(self):
        try:
            ...
            config_data["proxy"]["static_proxy"]["enable"] = (
                self.static_proxy_enable.isChecked()
            )
            ...
            config_data["proxy"]["static_proxy"]["username"] = (
                self.proxy_username.text()
            )
            config_data["proxy"]["static_proxy"]["password"] = (
            ...
            config_data["query"]["risk_delay_enable"] = (
                self.risk_delay_enable.isChecked()
            )
            config_data["query"]["risk_delay_seconds"] = self.risk_delay_seconds.value()
...
class MainWindow(QMainWindow):
    ...
    def parse_data_list(self, data_list):
        ...
        is_batch_result = any(
            item.get("_query_target") for item in data_list if isinstance(item, dict)
        )
        has_domain = any(
            item.get("domain") for item in data_list if isinstance(item, dict)
        )
        ...
        if is_batch_result:
            ...
            self.result_table.setHorizontalHeaderLabels(
                ["查询目标", name_header, "单位名称", "备案号"]
            )
        else:
            self.result_table.setColumnCount(3)
            self.result_table.setHorizontalHeaderLabels(
                [name_header, "单位名称", "备案号"]
            )
        ...
        for i, item in enumerate(data_list):
            ...
            name_value = (
                item.get("domain") if has_domain else item.get("serviceName", "")
            )
    ...
    def export_results(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self,
            "导出结果",
            f"icp_query_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
            "Excel文件 (*.xlsx);;所有文件 (*)",
        )
